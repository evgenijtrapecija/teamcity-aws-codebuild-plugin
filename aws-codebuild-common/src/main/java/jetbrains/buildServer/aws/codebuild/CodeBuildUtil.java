...
public final class CodeBuildUtil {
  ...
  public static String getBuildLogLink(@NotNull String buildId, @NotNull String projectName, @NotNull String region) {
    // Use the CloudWatch console URL format with proper encoding
    String encodedLogGroup = "$252Faws$252Fcodebuild$252F";
    String encodedStream = projectName + "$252F" + buildId.replace(projectName + ":", "");
    return String.format("https://%s.console.aws.amazon.com/cloudwatch/home?region=%s#logsV2:log-groups/log-group/%s/log-events/%s", region, region, encodedLogGroup, encodedStream);
  }
