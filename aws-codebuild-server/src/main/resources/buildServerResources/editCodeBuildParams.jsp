...
<l:settingsGroup title="AWS CodeBuild settings">
    ...
    <tr class="advancedSetting">
        <th><label for="${stream_logs_param}">${stream_logs_label}:</label></th>
        <td><props:checkboxProperty name="${stream_logs_param}"/>
            <span class="smallNote">Stream CloudWatch logs directly into the TeamCity build log for real-time monitoring</span>
        </td>
    </tr>
    <tr class="advancedSetting">
        <th><label for="${cloudwatch_region_param}">${cloudwatch_region_label}:</label></th>
        <td><props:textProperty name="${cloudwatch_region_param}" className="longField" maxlength="256"/>
            <span class="smallNote">Specify a different region for CloudWatch logs (e.g., eu-central-1). Leave empty to use the same region as CodeBuild.</span>
        </td>
    </tr>
    <tr class="advancedSetting">
        <th><label for="${debug_logs_param}">${debug_logs_label}:</label></th>
        <td><props:checkboxProperty name="${debug_logs_param}"/>
            <span class="smallNote">Enable debug logging for CloudWatch operations and troubleshooting</span>
        </td>
    </tr>
</l:settingsGroup>